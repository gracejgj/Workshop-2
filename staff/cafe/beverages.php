...
<div><center>
	<?php
		...
		$sql = "SELECT * from menu where mc_id='2' and cafe_id='1' LIMIT $offset, $total_records_per_page";
		...
		if ($result->num_rows > 0) {
			...
			while ($row = $result -> fetch_assoc()) {
				...
				echo "<td><img width='80px' height='80px' src='img/foods/".$row['image']."'></td>";
				echo "<td> ". $row['menu_name']. "</td>";
				echo "<td> ". $row['price']. "</td>";
				// UPDATE
					echo "<td><a class='button button4' href=beverages_update.php?pid=".$row['menu_id']." >UPDATE</a></td>" ;
					// DELETE
	...
	for ($counter = $total_no_of_pages - 5; $counter <= $total_no_of_pages; $counter++) {
	if ($counter == $page_no) {
	echo "<li class='active'><a>$counter</a></li>";	
	}else{
	echo "<li><a href='?page_no=$counter'>$counter</a></li>";
